...
- **Immich v3.2.0 or v3.2.1** (both tested). Earlier versions are not supported: v3.2.0 moved person identity into `person_group` and dropped `person.id`. v3.2.1 made no database schema changes, so the sidecar needs nothing extra for it. Later v3.2.x versions may work but the database schema can change between releases, so check the [Immich release notes](https://github.com/immich-app/immich/releases) before upgrading. Note: since v3, album ownership lives in `album_user` (role `owner`), and the sidecar copies OCR results (`asset_ocr`/`ocr_search`) and video stream metadata (`asset_video`/`asset_audio`/`asset_keyframe`) alongside CLIP embeddings.
...
4. **Upgrade Immich** to v3.2.1 (or v3.2.0) and let its migrations finish.
...
- **Direct database access**: This service writes directly to Immich's database. Tested with v3.2.0 and v3.2.1. Schema changes in other versions may require updates to this sidecar. Always back up your database before use.
