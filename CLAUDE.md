...
- `sync_engine.py` — Orchestrates the 6 phases, returns stats dict. Every phase runs through `_phase`, which logs a failure and lets the cycle continue: an unguarded exception skipped everything after it, so a deterministic failure in Phase 3 (which hardlinks files) meant Phase 4 never ran again. Phase 1 is guarded per *job*, so one job's bad source library doesn't cost the jobs behind it. `_phase` re-raises exactly two things — `SchemaValidationError` (every later phase is more SQL against a shape we no longer understand) and a broken connection (`sync_loop` needs to see it to reset the pool). `_cleanup_step` savepoints each Phase 4 step on top of that (naming it off `fn.__name__` rather than an interpolated literal). It and the other per-item savepoint guards (`sync_faces_for_asset_guarded`, `sync_asset`, `delete_target_asset`) re-raise when `conn.is_closed()`: a savepoint contains a failed statement, not a lost connection, and absorbing one ran every later step into a dead socket instead of letting `_phase` pass it up to `sync_loop`. The Phase 1 batch loop breaks when a full batch makes no progress, so a batch of persistently failing assets can't spin forever — it passes `limit=BATCH_SIZE` explicitly, because the loop's short-batch exit compares against `BATCH_SIZE` and the query's own default was a second copy of the number
- `confirm.py` — The destructive-action prompt `delete_synced.py` and `dedup_synced.py` share. An abbreviation must never resolve to the delete branch: both files used to accept `"d"`, which is neither `"cancel"` nor `"dry-run"`, so it fell through to the deletion
...
- `db.py` — asyncpg pool (min=2, max=10), `transaction()` context manager, query helpers. Two connection predicates, deliberately different: `is_connection_error` is narrow (`ConnectionError` plus asyncpg's own) and gates whether a failure kills the sync cycle, so a false positive is expensive — a full disk is an `OSError`, not a dead socket; `should_reset_pool` is broad (the whole `OSError` family) and only decides whether to rebuild the pool, where a false positive costs a reconnect
...
- `main.py` — Entry point: config validation, health wait, DB init, schema validation, tracking-table migrations, concurrent sync + scan loops. Schema validation runs *before* the migrations: `_migrate_v4` issues a DELETE against Immich's own `face_search`, and running that first meant an Immich upgrade produced a raw `UndefinedTableError` from inside a destructive statement instead of the curated breaking-change message. `_migrate_v4` flips `sourceType` before deleting the embeddings, both in one transaction — the reverse order leaves copies as `machine-learning` with no embedding, which is the exact state it exists to prevent
...
- `face_search.embedding` and `smart_search.embedding` are pgvector columns, but **not the same width**: the dumped fixture has `face_search` at `vector(512)` and `smart_search` at `vector(1152)`, because the CLIP one tracks whichever model the instance is configured for. Never assert a dimension anywhere — hardcoding 512 is what produced a false-positive schema failure when Immich changed CLIP models, and this line said both were 512 for a while after the fixture already disagreed
