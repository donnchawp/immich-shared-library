...
- `sync_engine.py` — Orchestrates the 6 phases, returns stats dict. `_cleanup_step` savepoints each Phase 4 step (naming it off `fn.__name__` rather than an interpolated literal). It and the other per-item savepoint guards (`sync_faces_for_asset_guarded`, `sync_asset`, `delete_target_asset`) re-raise when `conn.is_closed()`: a savepoint contains a failed statement, not a lost connection, and absorbing one ran every later step into a dead socket instead of letting `sync_loop` reset the pool. The Phase 1 batch loop breaks when a full batch makes no progress, so a batch of persistently failing assets can't spin forever
